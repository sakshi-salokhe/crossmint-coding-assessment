...
class CreateMetaverseClient:
    def __init__(self, api_client, goal_map):
        self.api_client = api_client
        self.goal_map = goal_map

    def create_metaverse(self):
        total_rows = len(self.goal_map)
        total_columns = len(self.goal_map[0])

        result_metaverse = [[0 for _ in range(total_columns)] for _ in range(total_rows)]

        for row in range(total_rows):
            for column in range(total_columns):
                current_cell_value = self.goal_map[row][column]
                result_metaverse[row][column] = current_cell_value
